      ...
      bottomNavigationBar: Container(
        ...
        child: SafeArea(
          child: Padding(
            ...
            child: Row(children: [
              _NavItem(icon: '🏠', label: 'Home', active: _tab == 0, onTap: () => setState(() => _tab = 0)),
              _NavItem(icon: '🛒', label: 'Shop', active: _tab == 1, onTap: () => setState(() => _tab = 1), badge: cartCount > 0 ? cartCount : null),
              _NavItem(icon: '🤖', label: 'AI', active: _tab == 2, onTap: () => setState(() => _tab = 2)),
              _NavItem(icon: '📈', label: 'Mandi', active: _tab == 3, onTap: () => setState(() => _tab = 3)),
              _NavItem(icon: '👤', label: 'Profile', active: _tab == 4, onTap: () => setState(() => _tab = 4)),
            ]),
          ),
        ),
      ),
        ...
        child: AnimatedContainer(
          ...
          child: Stack(alignment: Alignment.topCenter, children: [
            Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Text(label, style: TextStyle(fontSize: 10, fontWeight: active ? FontWeight.w700 : FontWeight.w400, color: active ? AppColors.primary : AppColors.textTertiary)),
            ]),
            if (badge != null) Positioned(right: 12, top: 0,
              child: Container(width: 16, height: 16, decoration: const BoxDecoration(color: AppColors.error, shape: BoxShape.circle),
          ...
          flexibleSpace: FlexibleSpaceBar(
            background: Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.end, children: [
                Row(children: [
                  Container(width: 44, height: 44, decoration: BoxDecoration(color: AppColors.amberLight, borderRadius: BorderRadius.circular(12)), child: const Center(child: Text('👨‍🌾', style: TextStyle(fontSize: 24)))),
                  const SizedBox(width: 12),
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('नमस्ते, $name 🙏', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: Colors.white)),
                    Text('${DateTime.now().toLocal().toString().split(' ')[0]}', style: TextStyle(fontSize: 12, color: Colors.white.withOpacity(0.6))),
                  ]),
                  const Spacer(),
        ...
        SliverToBoxAdapter(child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            GridView.count(
              ...
              crossAxisCount: 4, childAspectRatio: 0.9, mainAxisSpacing: 8, crossAxisSpacing: 8,
              children: [
                _QuickAction('🌿', 'Shop', () => context.push('/farmer/shop')),
                _QuickAction('🧪', 'Soil AI', () => context.push('/farmer/soil')),
                _QuickAction('🔬', 'Disease', () => context.push('/farmer/disease')),
                _QuickAction('📈', 'Mandi', () => context.push('/farmer/mandi')),
                _QuickAction('🌱', 'Crops', () => context.push('/farmer/crop-advisor')),
                _QuickAction('☁️', 'Weather', () => context.push('/farmer/weather')),
                _QuickAction('🤖', 'AI Chat', () => context.push('/farmer/kisan-ai')),
                _QuickAction('🏛️', 'Schemes', () => context.push('/farmer/schemes')),
              ],
            ),
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              ...
              TextButton(onPressed: () => context.push('/farmer/orders'), child: const Text('View All →')),
            ]),
            dashboard.when(
              loading: () => const AppShimmerList(itemCount: 3),
              error: (e, _) => AppErrorState(message: 'Could not load your orders', onRetry: () => ref.invalidate(farmerDashboardProvider)),
              data: (data) {
                final orders = data['recentOrders'] as List? ?? [];
                if (orders.isEmpty) return const AppEmptyState(icon: '📦', title: 'No recent orders', subtitle: 'Start exploring the shop!');
                return Column(children: orders.take(3).map((o) => _OrderTile(order: o)).toList());
              },
            ),
          ]),
        )),
...
class _OrderTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final statusColor = status == 'DELIVERED' ? AppColors.success : status == 'CANCELLED' ? AppColors.error : AppColors.amber;
    return Container(
      margin: const EdgeInsets.only(bottom: 8),
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(12), border: Border.all(color: AppColors.border)),
      child: Row(children: [
        Container(width: 40, height: 40, decoration: BoxDecoration(color: AppColors.primarySurface, borderRadius: BorderRadius.circular(10)), child: const Center(child: Text('📦', style: TextStyle(fontSize: 20)))),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('#${(order['id'] as String).substring((order['id'] as String).length - 6).toUpperCase()}', style: AppTextStyles.headingSM),
          Text('₹${order['totalAmount']}', style: AppTextStyles.priceSmall),
        ])),
        Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: statusColor.withOpacity(0.12), borderRadius: BorderRadius.circular(20)),
