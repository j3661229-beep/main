...
class _CropAdvisorState extends State<CropAdvisorScreen> {
      ...
      body: _recommendations.isNotEmpty 
        ...
        : SingleChildScrollView(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    const Text('Gemini AI Advisor', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)), 
                    const SizedBox(height: 4),
                    Text('Tell us about your farm setup and our AI will recommend the top high-yield crops.', style: TextStyle(color: Colors.white.withOpacity(0.9), fontSize: 13))
                  ]))
                ])
              ),
              ...
              _buildField('Location / District', _locCtrl, 'e.g. Pune, Nashik'),
              const SizedBox(height: 14),
              _buildField('Soil Type', _soilCtrl, 'e.g. Black, Red, Sandy'),
              const SizedBox(height: 14),
              _buildField('Season', _seasonCtrl, 'Kharif, Rabi or Zaid'),
              const SizedBox(height: 14),
              _buildField('Farm Size (Acres)', _sizeCtrl, 'Enter farm size in acres', isNum: true),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _loading 
                    ...
                    : const Text('✨ Get AI Recommendations', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                )
              ),
              const SizedBox(height: 40),
            ]),
  ...
  Widget _buildField(String label, TextEditingController ctrl, String hint, {bool isNum = false}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: AppTextStyles.labelLG.copyWith(color: AppColors.textSecondary)),
            ...
            return Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  ...
                  const SizedBox(width: 16), 
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(crop['crop'] ?? 'Unknown', style: AppTextStyles.headingMD),
                    const SizedBox(height: 4),
                    Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: AppColors.primaryLight, borderRadius: BorderRadius.circular(20)), child: Text('${crop['matchPercent']}% Match', style: const TextStyle(color: AppColors.primaryDark, fontWeight: FontWeight.bold, fontSize: 11))),
                  ])),
                ]),
                const SizedBox(height: 16),
                const Divider(),
                const SizedBox(height: 12),
                Text(crop['reason'] ?? '', style: AppTextStyles.bodyMD.copyWith(color: AppColors.textSecondary)),
  ...
  Widget _statInfo(String label, String val, {required String icon}) => Row(children: [
    Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: AppColors.background, borderRadius: BorderRadius.circular(8)), child: Text(icon, style: const TextStyle(fontSize: 16))), 
    const SizedBox(width: 8), 
    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(label, style: const TextStyle(fontSize: 10, color: AppColors.textTertiary, fontWeight: FontWeight.bold)), 
      Text(val, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.primaryDark))
    ])
  ]);
}
