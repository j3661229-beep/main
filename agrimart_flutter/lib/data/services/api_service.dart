...
class ApiService {
  ...
  late final Dio _dio = Dio(
    ...
    ..interceptors.add(PrettyDioLogger(
      requestHeader: false, requestBody: true, responseBody: true, error: true, compact: true,
    ));
  ...
  Future<Map> sendOTP(String phone, String role) async {
    final r = await _dio.post('/auth/send-otp', data: {'phone': phone, 'role': role});
    return r.data['data'];
  }

  Future<Map> verifyOTP({required String phone, required String otp, String? name, String? language, String? role}) async {
    final r = await _dio.post('/auth/verify-otp', data: {'phone': phone, 'otp': otp, 'name': name, 'language': language, 'role': role});
    return r.data['data'];
  }
  ...
  // ── Products ──────────────────────────────────────────────
  Future<Map> getProducts({String? category, String? search, String? sort, double? lat, double? lng, int page = 1}) async {
  ...
  // ── Orders ────────────────────────────────────────────────
  Future<Map> createOrder({required String deliveryAddress, double? lat, double? lng, String? notes}) async {
    final r = await _dio.post('/orders', data: {'deliveryAddress': deliveryAddress, 'deliveryLat': lat, 'deliveryLng': lng, 'notes': notes});
    return r.data['data'];
  }
  ...
  Future<Map> createRazorpayOrder(String orderId) async {
    final r = await _dio.post('/payments/create-order', data: {'orderId': orderId});
    return r.data['data'];
  }

  Future<Map> verifyPayment({required String razorpayOrderId, required String razorpayPaymentId, required String signature}) async {
  ...
  Future<Map> analyzeSoil(String imagePath) async {
    final formData = FormData.fromMap({'image': await MultipartFile.fromFile(imagePath, filename: 'soil.jpg')});
  ...
  Future<Map> detectDisease(String imagePath) async {
    final formData = FormData.fromMap({'image': await MultipartFile.fromFile(imagePath, filename: 'crop.jpg')});
  ...
  Future<Map> kisanChat({required String message, List? history}) async {
    final r = await _dio.post('/ai/chat', data: {'message': message, 'history': history ?? []});
    return r.data['data'];
  }

  Future<Map> getCropCalendar({String? month, String? district}) async {
    final r = await _dio.get('/ai/crop-calendar', queryParameters: {'month': month, 'district': district});
    return r.data['data'];
  }
